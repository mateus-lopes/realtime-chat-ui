...
jest.mock('@/components/ui/MobileInput.vue', () => ({
  ...
  props: ['modelValue', 'type', 'label', 'placeholder', 'error', 'disabled', 'required'],
...
describe('LoginForm', () => {
  ...
  it('renders login form correctly', () => {
    ...
    expect(wrapper.find('.form-subtitle').text()).toBe('Acesse sua conta para continuar');
  });
  ...
  it('validates password minimum length', async () => {
    ...
    expect(passwordInput.props('error')).toBe('Senha deve ter pelo menos 6 caracteres');
  });
